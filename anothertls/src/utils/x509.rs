    ...
    match der {
        ...
        EncodedForm::Primitive(prim) => match prim {
            ...
            DerType::ObjectIdentifier => {
                println!("GOT ObjectIdentifier");
                *consumed += size;
            }
            DerType::PrintableString => {
                println!("GOT PrintableString");
                *consumed += size;
            }
            DerType::OctetString => {
                println!("GOT OctetString");
                *consumed += size;
            }
            DerType::UTF8String => {
                println!("GOT UTF8String");
                *consumed += size;
            }
            DerType::UTCTime => {
                println!("GOT UTCTime");
            ...
            DerType::BitString => {
                println!("GOT BitString");
                if data[*consumed - 1] == 0x48 {
                    *consumed += 1;
                    // encaplusated
                    let size_should = size + *consumed - 1;
                    while size_should > *consumed {
                        parse(res, ParsingState::Value, data, consumed)?;
                    }
                } else {
                    // is subjectPublicKey
                    *consumed += size;
...
pub fn parse_x509(data: &[u8]) -> Result<X509, TlsError> {
    let mut res = X509Builder::new();
    let mut consumed = 0;
    parse(&mut res, ParsingState::Init, data, &mut consumed)?;
    println!("version={:?}", res.tbs_certificate.version);
    Ok(res.build())
}
...
mod tests {
    ...
    use crate::utils::x509::parse_x509;
